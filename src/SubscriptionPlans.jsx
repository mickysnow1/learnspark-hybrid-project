    ...
    <div className="w-full max-w-7xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {plansData.map((plan, index) => (
          // The hover effect is a gentle lift and a stronger shadow.
          <div 
            key={index} 
            className={`group relative rounded-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 hover:shadow-2xl dark:hover:shadow-emerald-900/50 ${plan.popular ? 'shadow-xl' : 'shadow-lg'}`}
          >
            <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-lg border border-white/20 rounded-2xl p-8 flex flex-col h-full text-center overflow-hidden">
              ...
              {/* This container now holds both the features and the overlay */}
              <div className="relative flex-grow mt-6">
                
                {/* The default feature list */}
                <ul className="space-y-3 text-gray-600 dark:text-gray-300 text-left transition-opacity duration-300 group-hover:opacity-0">
                  {plan.features.map((feature, i) => (<li key={i} className="flex items-center"><svg className="w-5 h-5 text-emerald-500 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg><span>{feature}</span></li>))}
                </ul>

                {/* --- THE NEW OVERLAY --- */}
                {/* It's positioned absolutely to cover the area, and fades in on hover */}
                <div className="absolute inset-0 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm p-4 text-left transition-opacity duration-300 opacity-0 group-hover:opacity-100 flex flex-col">
                    ...
                    <ul className="space-y-4 overflow-y-auto">
                        {plan.detailedPerks.map((perk, i) => (<li key={i}><p className="font-semibold text-gray-800 dark:text-gray-100">{perk.title}</p><p className="text-sm text-gray-600 dark:text-gray-400">{perk.text}</p></li>))}
                    </ul>
                </div>
